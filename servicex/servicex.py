...
class ServiceXDataset(ServiceXABC):
    ...
    def __init__(
        ...
        local_log: Optional[log_adaptor] = None,
        session_generator: Optional[
        ...
        config = (
            config_adaptor if config_adaptor is not None else ServiceXConfigAdaptor()
        )
    ...
    async def get_data_rootfiles_stream(
        self, selection_query: str, title: Optional[str] = None
    ...
    async def get_data_parquet_stream(
        self, selection_query: str, title: Optional[str] = None
    ) -> AsyncIterator[StreamInfoPath]:
        """Returns, as an async iterator, each completed batch of work from Servicex.
        The `StreamInfoPath` contains a path where downstream consumers can directly
    ...
    async def get_data_awkward_stream(
        ...
        """
        async for a in self._stream_return(
            selection_query, title, lambda f: self._converter.convert_to_awkward(f)
        ):
            yield a
    ...
    async def get_data_pandas_stream(
        ...
        """
        async for a in self._stream_return(
            selection_query, title, lambda f: self._converter.convert_to_pandas(f)
        ):
            yield a
    ...
    async def _file_return(
        ...
        return await self._data_return(
            selection_query, convert_to_file, title, data_format
        )

    @on_exception_itr(
        backoff.constant, ServiceXUnknownRequestID, interval=0.1, max_tries=3
    )
    @on_exception_itr(
        backoff.constant,
    ...
    async def _stream_url_buckets(
        ...
        async with aiohttp.ClientSession() as client:
            ...
            try:
                ...
                minio_files = self._get_minio_bucket_files_from_servicex(
                ...
                transform_status = await self._servicex_adaptor.get_query_status(
                    client, request_id
                )
                self._cache.remove_query(query)
                raise ServiceXFatalTransformException(
                    f'ServiceX Fatal Error: {transform_status["failure-info"]}'
                ) from e
                ...
                raise ServiceXException(
                    f"Failed to transform all files in {request_id}"
                ) from e
    ...
    async def _data_return(
        ...
        all_data = {
            f.file: f.data
            async for f in self._stream_return(
                selection_query, title, converter, data_format
            )
        }
    ...
    async def _stream_return(
        ...
        """
        as_data = (
            StreamInfoData(f.file, await asyncio.ensure_future(converter(f.path)))
            async for f in self._stream_local_files(selection_query, title, data_format)
        )  # type: ignore

        async for r in as_data:
            yield r

    async def _stream_local_files(
        ...
        as_files = (
            f
            async for f in self._get_files(
                selection_query, data_format, notifier, title
            )
        )  # type: ignore
    ...
    async def _get_request_id(
        self, client: aiohttp.ClientSession, query: Dict[str, Any]
    ) -> str:
        """
    ...
    async def _update_query_status(
        self, client: aiohttp.ClientSession, request_id: str
    ):
    ...
    async def _get_cached_files(
        self, cached_files: List[Tuple[str, Path]], notifier: _status_update_wrapper
    ):
        """
    ...
    async def _get_files_from_servicex(
        self,
        request_id: str,
        ...
        try:
            ...
            stream_downloaded = self._download_a_file(
                stream_new_object, request_id, minio_adaptor, notifier
            )
            ...
            logging.getLogger(__name__).info(
                f"Running servicex query for " f"{request_id} took {run_time}"
            )
            self._log.write_query_log(
                request_id,
                notifier.total,
                notifier.failed,
                run_time,
    ...
    async def _get_minio_bucket_files_from_servicex(
        self,
        request_id: str,
        client: aiohttp.ClientSession,
        minio_adaptor: MinioAdaptor,
        ...
        try:
            ...
            # Next, download the files as they are found (and return them):
            stream_new_object = find_new_bucket_files(
                minio_adaptor, request_id, stream_unique
            )
    ...
    def _build_json_query(
        ...
        logging.getLogger(__name__).debug(
            f"JSON to be sent to servicex: {str(json_query)}"
        )
