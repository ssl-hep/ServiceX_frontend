...
FuncADL expresses queries as a chain of functions applied to sequences of events or objects. Queries are written as typed Python objects and translated into C++ or Python depending on the source format. The following example fetches the {math}`p_T`, {math}`\eta`, and EM fraction of jets from an ATLAS PHYSLITE file:
